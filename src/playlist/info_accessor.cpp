...
QString infoAccessor::getInfo(QString filename, PlayListItem &res) {
    ...
    // 获取流数量
    res.setStreamNumbers(static_cast<int>(input_AVFormat_context_->nb_streams));
    // 计算视频长度
    int hours, mins, secs;
    secs = static_cast<int>(input_AVFormat_context_->duration / 1000000);
    mins = secs / 60;
    secs %= 60;
    hours = mins / 60;
    mins %= 60;
    ...
    qDebug() << "bit rate: " << static_cast<double>(input_AVFormat_context_->bit_rate) / 1000.0;

    const AVInputFormat *infoFormat = input_AVFormat_context_->iformat;
    res.setFormat(infoFormat->name);

    //分别遍历视频的流
    for (int i = 0; i < res.getStreamNumbers(); i++) {
        //取出一路流,并生成AVStream对象
        ...
        if (input_stream->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            //得到视频帧率
            res.setFrameRate(static_cast<int>(av_q2d(input_stream->avg_frame_rate)));
            ...
            res.setBitRate(static_cast<int>(codec_par->bit_rate / 1000));
            res.setVideoSize(static_cast<float>(res.getBitRate() * secs / (8.0 * 1024)));
